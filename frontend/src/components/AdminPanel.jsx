...
export default function AdminPanel() {
  ...
  const scrapeAndInsert = async () => {
    try {
      ...
      setStatus(`Tamamlandı. song_id: ${data?.songId || 'n/a'}`);
    } catch (err) {
  ...
  const quickInsertDemo = async () => {
    try {
      ...
      const { error: cErr } = await supabase.from('chords').insert({
        song_id: song.id,
        content: '[Am]Merhaba\n[C]Dünya',
      });
      if (cErr) throw cErr;
    ...
    <section className="mx-auto mt-8 max-w-xl rounded-xl border p-4">
      ...
      {!ok ? (
        ...
        <div className="space-y-3">
          ...
          />
          <button onClick={scrapeAndInsert} className="w-full rounded bg-blue-600 p-2 text-white">
            URL'den çek ve kaydet
          </button>
          <button onClick={quickInsertDemo} className="w-full rounded bg-emerald-600 p-2 text-white">
            Hızlı örnek kayıt ekle
          </button>
          {status ? <p className="rounded bg-slate-100 p-2 text-sm">{status}</p> : null}
        </div>
      )}
    </section>
  );
